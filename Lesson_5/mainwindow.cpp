...
{
    QString s = QFileDialog::getSaveFileName(this, tr("Окно сохранения"), QDir::current().path(), filter);
    if (s.length() > 0) {
...
{
    QString s = QFileDialog::getOpenFileName(this, tr("Окно открытия"), QDir::current().path(), filter);
    if(s.length() > 0) {
